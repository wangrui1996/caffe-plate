...
int main(int argc, char** argv) {
  ...
  std::vector<pair<std::string, std::string> > lines;
  std::string line;
  std::string filename;
  std::string label;
  size_t pos;
...
//    label = atoi(line.substr(pos + 1).c_str());
  ...
  CharacterDatum character_datum;
  Datum* datum = character_datum.mutable_datum();
  int count = 0;
  int data_size = 0;
  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
    ...
    string labelname = lines[line_id].second;
    ReadRichImageToCharacterDatum(filename, labelname, resize_height, resize_width, is_color, enc, &character_datum);
    ...
    // Put in db
    string out;
    CHECK(character_datum.SerializeToString(&out));
    txn->Put(key_str, out);
